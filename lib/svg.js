...
export function buildSvg({ track, artist, playing, albumDataUri, theme = 'dark' }) {
  ...
  const cardBg = light ? '#faf7f0' : '#1e1c1a'; // lifted charcoal, not pitch black -- keeps the vinyl's #0a0a0a fill clearly distinct
  ...
  <g class="vinyl-group ${playing ? '' : 'vinyl-paused'}">
    <circle cx="${VINYL_CX}" cy="${VINYL_CY}" r="${VINYL_SIZE / 2 - 0.75}" fill="#0a0a0a" stroke="rgba(255,255,255,0.16)" stroke-width="1.5" />
    ${grooves.join('\n    ')}
